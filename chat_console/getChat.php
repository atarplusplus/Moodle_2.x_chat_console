...
$userfrom = $DB->get_record('user',array('id'=>$_GET['sender']));
$userto = $DB->get_record('user',array('id'=>$_GET['receiver']));
...
{
    global $now, $a_day, $CFG, $DB;

    $select = '';
    $select.= "((useridto = ?";
    $select.= " AND useridfrom = ?) OR ";
    $select.= "(useridto = ?";
    $select.= " AND useridfrom = ?))";
    ...
    case 1:
        ...
        break;
    case 2:
        $params[] = $now - ($now % $a_day);
        break;
    default:
        return;
    }

    $outdated_messages = $DB->get_records_select('message', $select, $params, 'timecreated');

    foreach($outdated_messages as $message)
    {
        $messageid = $message->id;
        if ($DB->insert_record('message_read', $message) !== NULL)
        {
...
//                'timecreated');
        ...
        if($messages)
        {
            ...
            foreach($messages as $message)
            {
                ...
                $rec_message = $message->smallmessage;
                $rec_message = preg_replace("/[&]/", "#!amp!#", $rec_message);
                $rec_message = preg_replace("/[+]/", "#!pls!#", $rec_message);
                $username = $DB->get_field('user', 'firstname',
                        array('id'=>$message->useridfrom));
                $json .= '{';
                $json .= '"id":  "' . $message->id . '",';
